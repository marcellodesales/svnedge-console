 ...
 *
 */
final class TarGzCompressionCategory {
     ...
     */
    def static rightShift(File directory, File toFile)
        throws FileNotFoundException {

        if (!directory.exists()) {
            throw new FileNotFoundException("The source directory " +
                "'${directory}' does not exist!")
        } else if (toFile.exists() && toFile.isDirectory()) {
            throw new IllegalArgumentException("The destination file " + 
                "'${toFile}' is a directory!")
        }

        if (progressPrintStream) {
            progressPrintStream.println("Archiving the contents from the " +
                "directory '${directory}'")
            if (removeRootDir) {
